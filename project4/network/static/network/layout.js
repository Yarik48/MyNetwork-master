...
function follow_user(element, username, origin) {
    ...
    .then(() => {
        if(origin === 'suggestion') {
            element.parentElement.innerHTML = `<button class="btn btn-outline-secondary" type="button" onclick="unfollow_user(this,'${username}','suggestion')">Отписаться</button>`;
        }
        else if(origin === 'edit_page') {
            element.parentElement.innerHTML = `<button class="btn btn-outline-secondary float-right" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Отписаться</button>`;
        }
...
function unfollow_user(element, username, origin) {
    ...
    .then(() => {
        if(origin === 'suggestion') {
            element.parentElement.innerHTML = `<button class="btn btn-danger" type="button" onclick="follow_user(this,'${username}','suggestion')">Подписаться</button>`;
        }
        else if(origin === 'edit_page') {
            element.parentElement.innerHTML = `<button class="btn btn-danger float-right" onclick="follow_user(this,'${username}','edit_page')" id="follow-btn">Подписаться</button>`;
        }
        else if(origin === 'dropdown') {
            ///////////////////////////////////////////////////////////////////////////////////////////
        }
    ...
    eachrow.innerHTML = `
        <div>
            <a href='/${comment.commenter.username}'>
                <div class="small-profilepic" style="background-image: url(${comment.commenter.profile_pic});width:40px;height:40px"></div>
            </a>
        </div>
        <div style="flex: 1;">
